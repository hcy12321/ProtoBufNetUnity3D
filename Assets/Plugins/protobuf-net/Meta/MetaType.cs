...
namespace ProtoBuf.Meta
{
    ...
    public class MetaType : ISerializerProxy
    {
        ...
        private ValueMember AddField(int fieldNumber, string memberName, Type itemType, Type defaultType, object defaultValue)
        {
            ...
            var tmp = mi as PropertyInfo;
            if (tmp == null || !tmp.CanWrite)
            {
                // $"<{((PropertyInfo)mi).Name}>k__BackingField"
                var tmpStr = string.Format("<{0}>k__BackingField", ((PropertyInfo)mi).Name);
                var backingMembers = type.GetMember(tmpStr, Helpers.IsEnum(type) ? BindingFlags.Static | BindingFlags.Public : BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
